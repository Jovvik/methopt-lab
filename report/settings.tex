...
\newcommand{\stats}[1]{
    ...
    columns/length/.style={column name=$length$ \%, column type={|c}},
...
\newcommand{\graph}[1]{
    \begin{tikzpicture}
        \begin{semilogxaxis}[
            title = {График зависимости количества измерений от логарифма задаваемой точности $\varepsilon$},
        	xlabel = $\varepsilon$,
        	ylabel = $number$,
            ]
...
\newcommand{\mcode}[2]{
    \begin{code}
        \caption*{#1}
    	\inputminted[breaklines=true, xleftmargin=1em, linenos, frame=single, framesep=10pt, fontsize=\footnotesize, firstline=1, lastline=33]{haskell}{#2}
    \end{code}
}