...
Segment Dichotomy::step(Segment current_segment, func optimized_function) {
    ...
    return initial_step(new_start, new_end, optimized_function);
}
...
Segment Dichotomy::initial_step(double start, double end, func optimized_function) {
    double x1 = get_x1(start, end);
    double x2 = get_x2(start, end);
    return Segment(start, end, {{x1, optimized_function(x1)}, {x2, optimized_function(x2)}});
}

double Dichotomy::get_x1(double start, double end) { return (end + start - delta) / 2; }

double Dichotomy::get_x2(double start, double end) { return (end + start + delta) / 2; }